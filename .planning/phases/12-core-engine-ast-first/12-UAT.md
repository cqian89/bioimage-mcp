...
number: 1
name: Describe Meta Block
expected: |
  Run `bioimage-mcp describe scipy.ndimage.gaussian_filter`.
  The response includes a `meta` block with at least `tool_version` and
  `introspection_source`, and `params_schema` does NOT include metadata keys
  like `tool_version`, `introspection_source`, `callable_fingerprint`, or `module`.
