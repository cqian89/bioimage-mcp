...
def test_phasor_workflow_record_includes_provenance(tmp_path: Path) -> None:
    ...
    assert "resolved_params" in provenance
    assert "time_axis" in provenance["resolved_params"]
    assert "mapping_mode" in provenance

    outputs = record["outputs"]
    assert outputs["g_image"]["type"] == "BioImageRef"
    assert outputs["s_image"]["type"] == "BioImageRef"
    assert outputs["intensity_image"]["type"] == "BioImageRef"
